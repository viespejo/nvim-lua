...
-- DEBUG TEST ⬇️ using tressitter to find the closest test to the cursor
...
local function _debug_test(testname)
	dap.run({
		type = "go",
		name = testname,
		request = "launch",
...
local function debug_test()
	...
	print(msg)
	_debug_test(testname)
end

vim.keymap.set("n", "<leader>dt", debug_test, { noremap = true, silent = true, buffer = true })
